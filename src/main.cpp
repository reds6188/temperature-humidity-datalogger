...
void loop() {
	...
	if(TimerCloud.elapsedX1s(1)) {
		...
		Thing.createMessage();
		// Update Metrics ---------------------------------------------------------------------
		Thing.updateMetric(T5_S1_TEMP, sensor.getTemperature(0));
		Thing.updateMetric(T5_S1_HUMI, sensor.getHumidity(0));
		Thing.updateMetric(T5_S2_TEMP, sensor.getTemperature(1));
		Thing.updateMetric(T5_S2_HUMI, sensor.getHumidity(1));
		Thing.updateMetric(T5_S3_TEMP, sensor.getTemperature(2));
		Thing.updateMetric(T5_S3_HUMI, sensor.getHumidity(2));
		Thing.updateMetric(T5_S4_TEMP, sensor.getTemperature(3));
		Thing.updateMetric(T5_S4_HUMI, sensor.getHumidity(3));
